...
export default function POS() {
  ...
  const addToCart = (product) => {
    ...
    setCart((prev) => {
      ...
      return [...prev, {
        ...
        productId: product.id, name: product.name, img: product.img,
        price: product.price, type: sel.type, sizes: product.sizes, qty: sel.qty,
      }];
    });
  };
  ...
  return (
    <div className="pos">
      ...
      {modal === 'noName' && (
        <ValidationModal icon="person_off" iconColor="#BE1300" title="Customer Name Required" message="Please enter the customer's name in the Bill To field before placing an order." onClose={() => setModal(null)} />
      )}
      {modal === 'noCart' && (
        <ValidationModal icon="remove_shopping_cart" iconColor="#BE1300" title="Cart is Empty" message="Please add at least one product to the cart before placing an order." onClose={() => setModal(null)} />
      )}
      ...
      {modal === 'payref' && (
        <div className="pos__overlay">
          <div className="pos__payref-modal">
            ...
            <p className="pos__payref-sub">If you already have your reference number, kindly input it below.</p>
            <input
      ...
      {modal === 'success' && (
        <div className="pos__overlay">
          <div className="pos__success-modal">
            <div className="pos__success-circle">
              <span className="material-icons pos__success-check">check</span>
            </div>
        ...
        <div className="pos__card-info">
          ...
          <div className="pos__sel-grp">
            ...
            {sizeRows.map((row, ri) => (
              <div key={ri} className="pos__sel-row">
                {row.map((s) => (
                  <span key={s} className="pos__sel-btn pos__sel-btn--size pos__sel-btn--display">{s}</span>
                ))}
              </div>
            ))}
          </div>
...
function ConfirmOrderModal({ customerName, cart, paymentMethod, subtotal, total, onEdit, onConfirm }) {
  return (
    <div className="pos__overlay">
